...
optimism_eth_balance = {'id': 'ETH', 'symbol': 'ETH', 'name': 'Ethereum', 'native_balance': 0.23859492544325733, 'blockchain': 'Base', 'type': 'EVM'}
...
def test_sum_tokens_usd_balances():
    ...
    mon_wallet.add_json_entry(optimism_eth_balance)
    optimism_eth_balance2 = mon_wallet.update_all_missing_exchange_rate_via_parsed_quotes(parsed_quotes)
    assert mon_wallet.sum_balance_by_blockchain()['Base'] == cwl_entries['USD+']['usd_balance'] + cwl_entries['bsdETH']['usd_balance'] + optimism_eth_balance2['Base']['ETH'].usd_balance
