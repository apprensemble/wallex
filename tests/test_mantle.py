...
mantle_get_native = {"message": "OK", "result": "76269589638401015", "status": "1"}

def test_get_native_balance_from_blockscout():
    mantle_native_balance = mantle.parse_native_response_from_blockscout(mantle_get_native)
    assert mantle_native_balance['id'] == 'MNT'

def test_get_tokens_balance_from_blockscout():
    cwl_entries = mantle.parse_response_from_blockscout(mantle_get_tokens)
    print(cwl_entries)
    assert cwl_entries['USD+']['usd_balance'] == 432.04

def add_json_entries():
    cwl_entries = mantle.parse_response_from_blockscout(mantle_get_tokens)
    mantle_eth_balance = mantle.parse_native_response_from_blockscout(mantle_get_native)
...
def test_sum_tokens_usd_balances():
    ...
    mon_wallet.add_json_entry(mantle_eth_balance)
    mantle_native_balance2 = mon_wallet.update_all_missing_exchange_rate_via_parsed_quotes(parsed_quotes)
    assert mon_wallet.sum_balance_by_blockchain()[bc] == cwl_entries['USD+']['usd_balance'] + cwl_entries['bsdETH']['usd_balance'] + mantle_native_balance2[bc]['MNT'].usd_balance
