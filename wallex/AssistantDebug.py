...
class AssistantDebug:
  ...
  def __init__(self):
    ...
    self.active_data = {'nom_wallet':'custom_cwl','bc':'Base'}

  def define_active_data(self,nom_wallet,bc):
    self.active_data = {'nom_wallet':nom_wallet,'bc':bc}
    return self.active_data

  def set_active_data(self,nom_wallet=None,bc=None):
    if nom_wallet:
  ...
  def get_wallet(self,name_wallet=None):
    name_wallet,bc = self.set_active_data(name_wallet)
  ...
  def get_detailled_infos_from_wallet_and_bc_by_name(self,name_wallet=None,bc=None):
    name_wallet,bc = self.set_active_data(name_wallet,bc)
  ...
  def get_info_from_wallet_bc_info_criteria_by_name(self,name_wallet=None,bc=None,info=Info.USD,criteria=lambda x: x[1] > 0):
    name_wallet,bc = self.set_active_data(name_wallet,bc)
    wallet = self.get_wallet(name_wallet)
  ...
  def get_info_from_wallet_bc_info_criteria(self,wallet,bc,info=Info.USD,criteria=lambda x: x[1] > 0):
    data = self.ddata(self.li(bc,wallet),info)
    rf = filter(criteria,data.items())
    return list(rf)
